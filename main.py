...
OWNER_USERNAME = os.environ.get("OWNER_USERNAME","KGF_ROKY")
SUPPORT_GRP = os.environ.get("SUPPORT_GRP","https://t.me/singing_chating_fughting_group")
...
SOURCE = 'https://github.com/wwwlbs22/CHAT_BOT'
x=["❤️","🎉","✨","🪸","🎉","🎈","🎯"]
g=choice(x)
async def is_admins(chat_id: int):
    return [
        member.user.id
        async for member in linkxyyx.get_chat_members(
...
@linkxyyx.on_message(filters.incoming & filters.private, group=-1)
...
@linkxyyx.on_message(filters.command(["start",f"start@{BOT_USERNAME}"]))
async def restart(client, m: Message):
        accha = await m.reply_text(
                        text = f"{g}")
        await asyncio.sleep(1)
        await accha.edit("🦋𝗡𝗢𝗪 𝗖𝗢𝗠𝗘 𝗛𝗘𝗥𝗘 👉 @KGF_ROKY , @KGF_ROKY , @KGF_ROKY 𝗔𝗡𝗗 𝗠𝗔𝗞𝗘 𝗬𝗢𝗨𝗥 𝗚𝗜𝗥𝗟𝗙𝗥𝗜𝗡𝗗 🥀💋 𝗜𝗙 𝗬𝗢 𝗛𝗔𝗩𝗘 𝗔 𝗚𝗜𝗥𝗟𝗙𝗜𝗘𝗡𝗗  😘 𝗧𝗛𝗘𝗡 𝗬𝗢𝗨 𝗪𝗜𝗟𝗟 𝗚𝗜𝗙𝗧 🙊 𝗧𝗢 𝗠𝗬 𝗢𝗪𝗡𝗘𝗥 @JAYSHRI_RAM_JAYSHRI_RAM ❤️")
...
@linkxyyx.on_message(filters.command(["help", f"help@{BOT_USERNAME}"], prefixes=["","+", ".", "/", "-", "?", "$"]))
async def restart(client, message):
    hmm = await message.reply_photo(START_IMG,
                             caption= HELP_READ,
                        reply_markup= InlineKeyboardMarkup(HELP_BACK),
       )
@linkxyyx.on_message(filters.command(['source', 'repo']))
async def source(bot, m):
    await m.reply_photo(START_IMG, caption=SOURCE_TEXT, reply_markup=SOURCE_BUTTONS, reply_to_message_id=m.id)
#  alive
@linkxyyx.on_message(filters.command(["ping","alive"], prefixes=["","+", "/", "-", "?", "$", "&","."]))
...
async def vickai(client: Client, message: Message):
   ...
   if not message.reply_to_message:
       ...
       is_vick = vick.find_one({"chat_id": message.chat.id})
       if not is_vick:
           await linkxyyx.send_chat_action(message.chat.id, ChatAction.TYPING)
           K = []  
   ...
   if message.reply_to_message:  
       ...
       if not message.reply_to_message.from_user.id == bot_id:          
           ...
           if message.text:                 
               ...
               if not is_chat:
                   chatai.insert_one({"word": message
